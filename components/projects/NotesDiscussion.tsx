...
function NoteCard({ note, currentUserId, onDelete, onPin }) {
    ...
    return (
        <div className={`bg-white rounded-xl border transition-all hover:shadow-sm ${
            note.pinned ? 'border-yellow-300 ring-1 ring-yellow-200' : 'border-gray-200'
        }`}>
            <div className="p-5">
                ...
                {/* ── Note content ── */}
                <div className={`rounded-lg p-3 text-sm text-gray-700 whitespace-pre-wrap leading-relaxed ${
                    note.note_type === 'Blocker'
                            ...
                            >
                                {meta && (
                                    <span className={`w-1.5 h-1.5 rounded-full ${
                                        filterType === type ? 'bg-white' : meta.dot
                                    }`} />
                                )}
