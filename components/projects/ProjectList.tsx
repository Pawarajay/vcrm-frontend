...
const STATUS_CONFIG = {
    'Requirement': { bar: 'bg-purple-50 text-purple-700', dot: 'bg-purple-500', border: 'hover:border-purple-300', icon: Circle,       iconColor: 'text-purple-400',  pill: 'bg-purple-100 text-purple-800 border-purple-200' },
    ...
    'Delivered':   { bar: 'bg-green-50  text-green-700',  dot: 'bg-green-500',  border: 'hover:border-green-300',  icon: CheckCircle2, iconColor: 'text-green-500',   pill: 'bg-green-100  text-green-800  border-green-200'  },
...
function TableView({
    projects, onEdit, onDelete, onStatusChange, onProgressChange,
}: {
    projects: Project[];
    onEdit?: (p: Project) => void;
    onDelete?: (p: Project) => void;
                ...
                <table className="min-w-full divide-y divide-gray-100 text-sm">
                    <thead>
                        <tr>
                            ...
                            <Th label="Project Type"        col="service"            className="min-w-[120px]" />
                            ...
                            <Th label="Expected Delivery"   col="delivery_date"      className="min-w-[130px]" />
                            <Th label="Current Status"      col="status"             className="min-w-[130px]" />
                            <Th label="Priority"            col="priority"           className="min-w-[100px]" />
                            <Th label="Completion %"        col="progress_percentage" className="min-w-[130px]" />
                            <Th label="Project Description"                          className="min-w-[180px]" />
                            <Th label="Project Update"                               className="min-w-[180px]" />
                            <Th label="Notes"                                        className="min-w-[160px]" />
                            <Th label=""                                             className="w-16 sticky right-0 bg-gray-50" />
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {sorted.map((p, idx) => {
                            const statusCfg  = STATUS_CONFIG[p.status as keyof typeof STATUS_CONFIG] || STATUS_CONFIG['Requirement'];
                                ...
                                >
                                    ...
                                    <td className="px-3 py-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-1">
                                                {p.title}
                                            </span>
                                        </div>
                                        {p.task_count ? (
                                    ...
                                    {/* Client */}
                                    <td className="px-3 py-3">
                                        <span className="text-gray-700 text-xs">{p.client_name || '—'}</span>
                                    </td>

                                    {/* Project Type / Service */}
                                    <td className="px-3 py-3">
                                        {p.service ? (
                                            <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-gray-100 text-gray-700 text-xs font-medium border border-gray-200">
                                                {p.service}
                                            </span>
                                    ...
                                    <td className="px-3 py-3" onClick={e => e.stopPropagation()}>
                                        {onStatusChange ? (
                                            ...
                                            <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold border ${statusCfg.pill}`}>
                                                {p.status}
                                            </span>
                                        )}
                                    </td>
                                    ...
                                    <td className="px-3 py-3 max-w-[180px]">
                                        {p.project_update ? (
                                            <span className="text-xs text-blue-700 bg-blue-50 px-2 py-1 rounded-lg line-clamp-2 block border border-blue-100">
                                                {p.project_update}
                                            </span>
                                        ) : <span className="text-gray-400 text-xs">—</span>}
                                    </td>
    ...
    return (
        <div className={`bg-white rounded-xl shadow-sm hover:shadow-md transition-all border border-gray-200 ${cfg.border} flex flex-col`}>
                ...
                {(pct > 0 || onProgressChange) && (
                    <div onClick={e => e.stopPropagation()}>
                        ...
                        {onProgressChange ? (
                            ...
                            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                <div className={`h-full rounded-full ${pct >= 100 ? 'bg-green-500' : 'bg-[#3A7AFE]'}`} style={{ width: `${pct}%` }} />
                            </div>
                        )}
                    </div>
                )}
            ...
            <div className="px-5 py-3 border-t border-gray-100 grid grid-cols-2 gap-3">
                ...
                <div>
                    <p className={`text-[10px] mb-0.5 uppercase tracking-wide ${overdue ? 'text-red-400' : 'text-gray-400'}`}>Go-Live</p>
                    <p className={`text-xs font-medium ${overdue ? 'text-red-600' : 'text-gray-700'}`}>{fmtDate(project.delivery_date)}</p>
                </div>
            </div>
...
interface Filters { status: string; priority: string; service: string; client: string; }

function FilterBar({ filters, onChange, clientOptions, onClear }: {
    filters: Filters; onChange: (f: Filters) => void; clientOptions: string[]; onClear: () => void;
}) {
    const [open, setOpen] = useState(false);
    const hasActive = filters.status !== 'All' || filters.priority !== 'All' || filters.service !== 'All' || filters.client !== 'All';

    return (
        <div className="relative">
            <button onClick={() => setOpen(o => !o)}
                className={`flex items-center gap-2 px-3 py-2 border rounded-xl text-sm transition-colors ${hasActive ? 'border-[#3A7AFE] bg-blue-50 text-[#3A7AFE]' : 'border-gray-200 bg-white text-gray-600 hover:bg-gray-50'}`}>
                <SlidersHorizontal size={15} />
                Filters
                {hasActive && <span className="w-2 h-2 bg-[#3A7AFE] rounded-full" />}
                <ChevronDown size={13} className={`transition-transform ${open ? 'rotate-180' : ''}`} />
            ...
            {open && (
                <>
                    ...
                    <div className="absolute left-0 top-11 z-20 bg-white border border-gray-200 rounded-xl shadow-lg p-4 w-72 space-y-4">
                        ...
                        <FilterGroup label="Service">
                            <select value={filters.service} onChange={e => onChange({ ...filters, service: e.target.value })}
                                className="rounded-xl border border-gray-200 px-3 py-2 text-sm w-full focus:border-[#3A7AFE] focus:outline-none bg-white text-gray-900 h-9">
                                <option value="All">All services</option>
                                {SERVICE_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
...
function Chip({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
    return (
        <button onClick={onClick}
            className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${active ? 'bg-[#3A7AFE] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            {children}
        </button>
    );
}
...
export default function ProjectList({ projects, onUpdate, onEdit, onDelete, onStatusChange, onProgressChange }: ProjectListProps) {
    ...
    const clearFilters = () => { setSearch(''); setFilters({ status: 'All', priority: 'All', service: 'All', client: 'All' }); };
    const hasAnyFilter = search !== '' || Object.values(filters).some(v => v !== 'All');
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                {/* View toggle */}
                <div className="flex items-center gap-0 border border-gray-200 rounded-xl overflow-hidden bg-white h-9 shrink-0">
                    <button
                        onClick={() => setViewMode('table')}
                        title="Table view"
                        className={`flex items-center gap-1.5 px-3 py-2 text-sm transition-colors ${viewMode === 'table' ? 'bg-[#3A7AFE] text-white' : 'text-gray-500 hover:bg-gray-50'}`}
                    >
                        <Table2 size={15} />
                    ...
                    <button
                        ...
                        title="Card view"
                        className={`flex items-center gap-1.5 px-3 py-2 text-sm transition-colors ${viewMode === 'card' ? 'bg-[#3A7AFE] text-white' : 'text-gray-500 hover:bg-gray-50'}`}
                    >
                        <LayoutGrid size={15} />
            ...
            {filtered.length === 0 ? (
                <div className="bg-white rounded-2xl border border-dashed border-gray-300 p-14 text-center">
                    ...
                    <p className="text-gray-500 font-medium">No projects match your search</p>
                    <p className="text-gray-400 text-sm mt-1">Try different keywords or clear the filters.</p>
                    <button onClick={clearFilters} className="mt-4 px-4 py-2 bg-[#3A7AFE] text-white text-sm rounded-xl hover:bg-[#2563EB]">Clear filters</button>
                </div>
            ) : viewMode === 'table' ? (
                <TableView
                    projects={filtered}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {filtered.map(p => (
                        <ProjectCard key={p.id} project={p} onEdit={onEdit} onDelete={onDelete}
                            onStatusChange={onStatusChange} onProgressChange={onProgressChange} />
                    ))}
                </div>
            )}
        </div>
