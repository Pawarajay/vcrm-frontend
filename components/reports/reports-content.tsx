...
// ─────────────────────────────────────────────────────────────────────────────
// REPORTS & ANALYTICS — redesigned to match the DashboardContent visual
// language (gradient KPI tiles, animated charts, reveal-on-mount panels) and
// to fix the bugs found while reviewing the previous version:
//
// 1. The period selector (7/30/90 days) used to only affect the three top
//    KPI cards — every chart below kept showing all-time data regardless of
//    what was selected. Every period-scoped section below now actually
//    respects `period`; sections that are intentionally snapshots of
//    *current* state (open pipeline, active clients) are explicitly
//    labeled "Current" / "All time" instead of being silently ambiguous.
//
// 2. RESOLVED — status-vocabulary mismatch: this file's lead statuses
//    ("qualified-lead" / "free-inspection" / "quotation" / "installation" /
//    "closed") didn't match DashboardContent's ("lead" / "demo" /
//    "proposal" / "negotiation" / "won"), so the conversion funnel and
//    channel-performance conversion rates were silently showing 0 for
//    everything — every lead was falling through every status check.
//    `classifyStatus()` below now recognizes both vocabularies, falls back
//    to fuzzy matching for near-variants, and surfaces any status value it
//    still can't classify directly in the funnel panel instead of hiding
//    it. If you see an "Unmapped" line in the funnel, that's your signal —
//    send me the exact strings and I'll add them to EXACT_STAGE.
//
// 3. All chart primitives now come from `chart-kit` instead of being
//    duplicated — see that file's header comment.
// ─────────────────────────────────────────────────────────────────────────────
...
import {
  ...
  Target, Activity, Download, ShieldAlert, Sparkles, Phone, HelpCircle,
} from "lucide-react"
import {
...
// Exact, known status strings — covers this file's original vocabulary and
// DashboardContent's vocabulary, so whichever one your real data actually
// uses, it resolves correctly without guessing.
...
// Substring fallback for near-variants ("Proposal Sent", "Negotiating",
// "Won Deal", etc.) that don't match an exact key above.
...
// ─── CSV export ───────────────────────────────────────────────────────────────
// Exports every section currently on the page, not just the raw lead list —
// the previous version only wrote `periodLeads` through six fields, so an
// export "felt thin" next to everything actually shown on screen. This
// writes one CSV with a titled section per panel; opens cleanly in Excel/
// Sheets with blank-line separators between sections.
...
export function ReportsContent() {
  ...
  // ── Period-scoped slices (the actual fix: everything below now reads
  // from these instead of the raw `leads`/`invoices` arrays) ─────────────────
  ...
  // Both now go through classifyStatus() instead of comparing against a
  // single hardcoded string, which is what made conversions read as 0%.
  ...
  // ── Headline KPIs (period-scoped, with trend vs the immediately
  ...
  // ── Risk / attention metrics (current state, not period-scoped) ───────────
  ...
  // ── Channel performance — period-scoped, conversion % not just volume ─────
  ...
  // ── Executive summary — narrative takeaways instead of making the
  // reader assemble the story themselves from six charts ────────────────────
  ...
  // ── Service breakdown (period-scoped) ───────────────────────────────────────
  ...
  // ── Source distribution (period-scoped) ─────────────────────────────────────
  ...
  // ── Monthly lead volume — fixed 6-month trend, intentionally independent
  // of the period selector (it's a long-range trend, not a period KPI) ───────
  ...
  // ── Funnel — current pipeline snapshot, all-time by design. Dynamically
  // bucketed via classifyStatus() instead of hardcoded keys, so it actually
  // reflects whatever status strings your data really uses. Anything that
  // can't be classified shows up in `unmappedStatuses` below the chart
  // instead of silently vanishing into a zero. ────────────────────────────────
  ...
  // ── Open-lead priority breakdown — current snapshot ─────────────────────────
  ...
  // ── Clients by service — current state, all time ────────────────────────────
  ...
  // ── Top open leads by value — current snapshot ──────────────────────────────
  ...
  // ── Top clients by revenue (for the concentration panel) ────────────────────
    ...
    <div className="min-h-screen bg-[#F4F6FB]">
        ...
        <div className="max-w-screen-xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button
              ...
              className="rounded-xl border-gray-200 text-gray-600 text-xs gap-1.5 h-8"
            >
            ...
            <Button
              ...
              className="rounded-xl border-gray-200 text-gray-600 text-xs gap-1.5 h-8 disabled:opacity-50"
            >
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-5 sm:py-6 space-y-6 sm:space-y-7">
        ...
        <Reveal delay={60}>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <GradientTile label="New leads" value={kpis.newLeads} trend={kpis.newLeadsTrend} gradient={TILE_GRADIENTS[0]} />
            <GradientTile label="Conversion rate" value={kpis.convRate} isPercent gradient={TILE_GRADIENTS[1]} trend={kpis.convRateTrend} />
            <GradientTile label="Revenue collected" value={kpis.revenue} isMoney trend={kpis.revenueTrend} gradient={TILE_GRADIENTS[2]} />
            <GradientTile label="Active clients" value={kpis.activeClients} trend={kpis.newClientsTrend} gradient={TILE_GRADIENTS[3]} />
          </div>
        </Reveal>
        ...
        <Reveal delay={100}>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <GradientTile label="Open pipeline value" value={kpis.pipelineValue} isMoney gradient={TILE_GRADIENTS[0]} />
            <GradientTile label="Open leads" value={kpis.openLeadsCount} gradient={TILE_GRADIENTS[1]} />
            <GradientTile label="Avg. deal value" value={kpis.avgDealValue} isMoney gradient={TILE_GRADIENTS[2]} />
            <GradientTile label="Revenue concentration" value={attention.top3Share} isPercent gradient={TILE_GRADIENTS[3]} />
          </div>
        </Reveal>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            ...
            <Panel title="Open-lead priority" sub="Current snapshot — open leads only" icon={AlertCircle} accentColor={WARN}>
              ...
              <div className="mt-4 pt-3 border-t border-gray-50 flex items-center justify-between">
                ...
                <span className={`text-sm font-bold ${attention.followUpsOverdue > 0 ? "text-red-600" : "text-gray-800"}`}>
