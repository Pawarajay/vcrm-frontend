    ...
    >
      ...
      <div className="p-3.5 space-y-3">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            <GripVertical className="h-3.5 w-3.5 text-gray-300 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
            {/* UI CHANGE: flat blue avatar instead of gradient */}
...
// ─── Stat Card ────────────────────────────────────────────────────────────────
// UI CHANGE: compact card with left accent border + trend line
...
function StatCard({
  ...
  borderClass: string;   // full border color
}) {
  return (
    <div className={cn(
...
export function LeadsContent() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* ── Top Bar — Row 1: Title + View Toggle + Add Button ─────────── */}
      {/* UI CHANGE: Split into two rows. Row 1 = title + actions */}
      ...
      <div className="flex-1 overflow-auto px-6 py-4 space-y-4">
        ...
        {/* ── Stats row ─────────────────────────────────────────────────── */}
        {/* UI CHANGE: compact StatCard with left accent border + trend */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <StatCard
        ...
        {view === "table" && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            {/* UI CHANGE: darker column headers for scannability */}
            <div
            ...
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-gray-300">
                ...
                {(search || activeFilters > 0) ? (
                  ...
                  // UI CHANGE: CTA button on empty state
                  <Button
                    onClick={() => setAddOpen(true)}
                ...
                {filtered.map((lead, idx) => {
                  ...
                  return (
                    <div
                      key={lead.id}
                      onDoubleClick={() => open(lead)}
                      // UI CHANGE: zebra striping + left accent bar on hover
                      className={cn(
                      ...
                      style={{ gridTemplateColumns: "2.5fr 1fr 1.2fr 1.2fr 1fr 1.2fr 100px" }}
                    >
                      {/* UI CHANGE: left accent bar on hover */}
