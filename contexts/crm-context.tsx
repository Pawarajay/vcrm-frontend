...
// // ─── normalizeCustomer ────────────────────────────────────────────────────────
// // Renewal fields (nextRenewalDate, defaultRenewalStatus, defaultRenewalReminderDays,
// // defaultRenewalNotes) have been removed — they no longer exist in the DB schema.
...
//   // Legacy Vasify pricing (kept for backend compat)
...
//     // Preserve snake_case originals so existing components don't break
...
//     // next_action — preserved from DB so InlineNextActionDropdown reads correctly
//     next_action:         raw.next_action           ?? null,

//     sales_form_data: salesFormData,

//     // Lead form draft fields — read by SalesFormDialog
...
//       const normalized = (res.leads || []).map(normalizeLead)
//       setLeads(normalized)
//     } catch (err) {
...
//             : (() => { try { return JSON.parse(rr.reminderDays ?? rr.reminder_days ?? "[]") } catch { return [] } })(),
//         }))
//       )
//     } catch (err) {
...
//   try {
//     const base  = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api"
//     // const res = await retainersApi.getAll({ limit: 200 })
// const token = typeof window !== "undefined" ? localStorage.getItem("token") : null
//     if (!token) { setRetainers([]); return }

//     const res = await retainersApi.getAll({ limit: 200 })
//     const normalized: Retainer[] = (res.retainers || []).map((raw: any): Retainer => ({
//       id:            String(raw.id),
//       clientName:    raw.client_name    ?? raw.clientName    ?? "",
//       customerId:    raw.customer_id    ?? raw.customerId    ?? null,
//       service:       raw.service        ?? "other",
//       monthlyAmount: Number(raw.monthly_amount ?? raw.monthlyAmount ?? 0),
...
//       const payload = {
//         ...data,
//         totalValue: data.totalValue != null && (data.totalValue as any) !== "" ? Number(data.totalValue) : 0,
//       }
...
//         setInvoices((prev) => [normalizeInvoice((res as any).invoice, { customerId: c.id, customerName: c.name }), ...prev])
//       }
...
//         payload.totalValue = payload.totalValue != null && payload.totalValue !== "" ? Number(payload.totalValue) : 0
//       }
...
//   const addLead = async (data: Omit<Lead, "id" | "createdAt" | "updatedAt">): Promise<boolean> => {
//     try {
...
//         switch (key) {
//           // ── Sales form data: always serialise to string, send as snake_case ──
//           case "sales_form_data":
//           case "salesFormData": {
//             const serialised = typeof value === "string"
//               ? value
//               : JSON.stringify(value)
//             payload["sales_form_data"] = serialised
//             break
//           }

//           // ── Next action: always send as snake_case ───────────────────────
//           case "next_action":
//           case "nextAction":
//             payload["next_action"] = value
//             break

//           // ── Lead form draft: serialise objects, send as snake_case ────────
//           case "lead_form_draft":
//           case "leadFormDraft": {
//             const serialised = typeof value === "string"
//               ? value
//               : JSON.stringify(value)
//             payload["lead_form_draft"] = serialised
//             break
//           }

//           // ── lead_form_is_draft / lead_form_saved_at: pass through as snake_case ─
...
//           // ── Dates: normalise to YYYY-MM-DD in local timezone ─────────────
...
//       alert(err?.message || "Failed to convert lead to patient")
//       return false
//     }
//   }
...
//       items: data.items?.map(({ description, quantity, rate, amount }) => ({ description, quantity, rate, amount })) ?? [],
//     }
//   }
...
//       const normalized = normalizeInvoice(res.invoice, { ...data, status: data.status ?? fallbackStatus })
...
// addRetainer, updateRetainer, deleteRetainer, refreshRetainers,
// addRetainerPayment, updateRetainerPayment,
//     refreshRenewals, refreshUsers,
//     refreshDeals, refreshTasks,
//   }
